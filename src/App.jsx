...
const DemonOracle=({result,onClose,onShare,mode,aeonTotal,lightMode})=>{const[vis,setVis]=useState(false);useEffect(()=>{hapticHeavy();setTimeout(()=>setVis(true),100);},[]);if(!result)return null;const ang=result.type==="angelic";const d=result.demon;const isSub=mode==="subdecadence";const ac=ang?"#ffd700":"#ff0044";const lm=lightMode;const Sec=({label,children})=>(<div style={{borderTop:"1px solid "+(lm?ac+"30":ac+"18"),paddingTop:12,marginBottom:12}}><div style={{color:ac,fontSize:12,letterSpacing:3,marginBottom:6}}>{label}</div><div style={{color:lm?"#222":"#ddd",fontSize:17,lineHeight:1.85}}>{children}</div></div>);
  ...
  return(<div style={{position:"fixed",inset:0,zIndex:1000,background:lm?"rgba(255,255,255,0.96)":"rgba(0,0,0,0.94)",display:"flex",alignItems:"center",justifyContent:"center",opacity:vis?1:0,transition:"opacity 0.8s",backdropFilter:"blur(10px)",padding:12}}><div onClick={e=>e.stopPropagation()} style={{maxWidth:420,width:"100%",maxHeight:"90vh",overflowY:"auto",background:lm?"#fff":"linear-gradient(180deg,#0a0a0a,#050510)",border:"1px solid "+ac+"30",borderRadius:4,padding:"24px 20px",fontFamily:"'Courier New',monospace",boxShadow:lm?"0 4px 20px rgba(0,0,0,0.12)":"0 0 50px "+ac+"15",position:"relative",WebkitOverflowScrolling:"touch",color:lm?"#222":"#ccc"}}>{!lm&&<div style={{position:"absolute",inset:0,pointerEvents:"none",background:"repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(255,255,255,0.015) 2px,rgba(255,255,255,0.015) 4px)"}}/>}
    {ang?(<>
      ...
      <div style={{color:"#777",fontSize:13,marginBottom:16}}>Mesh-{d.mesh} · {d.type} · [{d.netSpan}] · <span style={{color:"#555",fontStyle:"italic"}}>{demonPhoneme(d.netSpan)}</span></div>
      <Sec label="PITCH">{d.pitch}</Sec>
      <Sec label={isSub?"NET-SPAN PASSAGE":"ZONE PASSAGE"}>{(()=>{const[a,b]=d.netSpan.split("::").map(Number);const za=ZONES[a];const zb=ZONES[b];return(<div><div style={{marginBottom:6}}>{d.zone}</div><div style={{fontSize:13,color:"#777",lineHeight:1.7}}><span style={{color:ac}}>{za.name}</span> ({za.region} · {za.planet}) → <span style={{color:ac}}>{zb.name}</span> ({zb.region} · {zb.planet})</div></div>);})()}</Sec>
      ...
      <div style={{position:"relative",zIndex:2,maxWidth:400,margin:"0 auto",padding:"6px 8px 10px",minHeight:"100dvh",overflow:gamePhase==="menu"?"auto":"auto"}}>
        ...
        {gamePhase==="menu"&&(
          <div style={{textAlign:"center",paddingTop:12}}>
            ...
            {/* ZONES — collapsible */}
            <button onClick={()=>{haptic();setShowZones(!showZones);}} style={{padding:"5px 14px",background:"transparent",border:"1px solid "+accent+"30",color:accent,fontFamily:"monospace",fontSize:10,letterSpacing:2,cursor:"pointer",borderRadius:2,display:"block",margin:"0 auto 12px"}}>{showZones?"HIDE ":""}NUMOGRAM ZONES</button>
            {showZones&&(<div style={{padding:"14px 12px",textAlign:"left",border:"1px solid #1a1a1a",borderRadius:2,background:"rgba(0,0,0,0.3)",marginBottom:16,maxHeight:400,overflowY:"auto"}}>
              <div style={{color:accent,fontSize:11,letterSpacing:3,marginBottom:12}}>◈ TEN ZONES · DECIMAL LABYRINTH ◈</div>
              {Object.keys(ZONES).map(k=>{const z=ZONES[k];const regionColor=z.region==="Plex"?"#9966ff":z.region==="Warp"?accent:"#0f0";return(<div key={k} style={{marginBottom:14,paddingBottom:10,borderBottom:"1px solid #111"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <span style={{color:regionColor,fontSize:14,fontWeight:"bold"}}>Zone-{k}</span>
                  <span style={{color:"#555",fontSize:10}}>{z.region} · {z.planet} · {z.spine}</span>
                </div>
                <div style={{color:"#666",fontSize:11,marginBottom:4}}>Syzygy: {k}::{z.twin} · {z.current} · {z.gate} · Phoneme: <span style={{fontStyle:"italic"}}>{z.phoneme}</span> · Imps: {z.impulses}</div>
                <div style={{color:"#999",fontSize:13,lineHeight:1.7}}>{z.desc}</div>
              </div>);})}
            </div>)}
